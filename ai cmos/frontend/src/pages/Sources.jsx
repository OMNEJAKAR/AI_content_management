  ...
  const handleSelectFolder = async (folderHandle) => {
    try {
      ...
      if (permission !== 'granted') {
        console.error('Folder permission not granted');
        return;
      }
      ...
      // ✅ Save in per-user storage with both keys to ensure consistency
      localStorage.setItem(`localSources_${currentUser}`, JSON.stringify(updated));
      
      const userObj = localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user')) : null;
      if (userObj?.username) {
        localStorage.setItem(`localSources_${userObj.username}`, JSON.stringify(updated));
      }
